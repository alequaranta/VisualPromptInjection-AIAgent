...
async def esegui_singolo_test(run_id):
    print(f"\n[Test {run_id}/{NUMERO_TEST}] Avvio con OpenAI (Vision attivo)...")

    # Wrapper nativo di browser-use per OpenAI
    llm = ChatOpenAI(
        model="gpt-5",
        temperature=0.0
    )
...
async def main():
    # Creiamo o svuotiamo il file di log per questa sessione di test con OpenAI
    log_file_path = "risultati_test_openai.txt"
    with open(log_file_path, "w", encoding="utf-8") as f:
        f.write("=== REPORT DI TEST AUTOMATIZZATO CON OPENAI ===\n\n")
