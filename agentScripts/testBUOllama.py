...
async def esegui_singolo_test(run_id):
    print(f"\n[Test {run_id}/{NUMERO_TEST}] Avvio con Ollama...")

    # Wrapper nativo di browser-use per Ollama
    llm = ChatOllama(
        model="qwen2.5vl:7b",
        ollama_options={"temperature": 0.0, "num_ctx": 16384}
    )
...
async def main():
    # Creiamo o svuotiamo il file di log per questa sessione di test con Ollama
    log_file_path = "risultati_test_ollama.txt"
    with open(log_file_path, "w", encoding="utf-8") as f:
        f.write("=== REPORT DI TEST AUTOMATIZZATO CON OLLAMA ===\n\n")
